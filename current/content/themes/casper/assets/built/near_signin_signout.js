(() => {
    ...
    let $signInSignOutBtn, $getAccountBalanceInput, $getAccountDetailsInput,
        ...
        $checkTransactionButton, $parseContractMethodsInput, $parseContractMethodsSubmitButton,
        $callChangeMethod, $parseContractView,
        tokensToSend, receiver, contractToParse;
    ...
    const parseNearContract = async () => {
        $parseContractView = $(createDataSelector(selectors.parseContractView));
        const { parseContract } = nearContractParser;
        const { code_base64 } = await nearAPI.connection.provider.query({
            account_id: contractToParse,
            finality: 'final',
            request_type: 'view_code',
        });
        if ($parseContractView.length) {
            $parseContractView.text(JSON.stringify(parseContract(code_base64)))
        }
    }
    ...
    const onClickHandler = async (event) => {
        ...
        if (!isFetching) {
            ...
            } else if (id === selectors.callChangeMethod) {
                const methodName = $(event.target).data('change-method');
                const data = await nearContract[methodName]();
            } else if (id === selectors.parseContractSubmit) {
                parseNearContract();
            }
            isFetching = false;
        }
    ...
    const onInputHandler = event => {
        ...
        if (dataID === selectors.receiverId) {
            receiver = value;
        } else if (dataID === selectors.tokensToSend) {
            tokensToSend = value;
        } else if (dataID === selectors.parseContractMethods) {
            contractToParse = value;
        }
    }
    ...
    const onClickCheckTransactionHandler = () => window.open(`https://explorer.testnet.near.org/accounts/${nearWallet.getAccountId()}`)
    ...
    const parseContractMethods = () => {
        $parseContractMethodsInput = $(createDataSelector(selectors.parseContractMethods));
        $parseContractMethodsSubmitButton = $(createDataSelector(selectors.parseContractSubmit));
        if ($parseContractMethodsInput.length) {
            $parseContractMethodsInput
                .on('input', onInputHandler)
                .on('keydown', onKeyDownHandler)
        }
        if ($parseContractMethodsSubmitButton.length) {
            $parseContractMethodsSubmitButton.on('click', onClickHandler)
        }
    }
    ...
    const onReadyHandler = () => {
        ...
        $callChangeMethod = $(createDataSelector(selectors.callChangeMethod));
        ...
        if ($callChangeMethod.length) {
            $callChangeMethod.on('click', onClickHandler)
        }
    }
    $(onReadyHandler);
})();
